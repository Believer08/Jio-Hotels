...
class STB_Mapping(BasePage):
    ...
    def goToSTB_Mapping(self):
        ...
        return STB_Mapping(self.driver)
    ...
    def deleteCreatedSubProperty(self):
        ...
        self.click(self.subPropertyList)
        self.wait_for_page_load()
        self.click((By.XPATH, f"//td[.='{r_subPropertyName}']//ancestor::tr//following::td//div//button[@data-bs-target='#deletePropertyModal']"))
        self.click(self.deleteSubPropertyBtn)
        self.wait_for_page_load()
        return STB_Mapping(self.driver)

    def deleteSubProperties(self):
        self.click(self.chooseHotel)
        self.click(self.mainProperty1st)
        self.click(self.subPropertyList)
        self.wait_for_page_load()
        self.send_keys(self.searchBox, "auto")
        c = self.return_elements_count((By.XPATH, "//button[@data-bs-target='#deletePropertyModal']"))
        self.goToSTB_SubPropertyList()
        if c >=1:
            for i in range(1, c + 1):
                self.send_keys(self.searchBox, "auto")
                self.wait_for_page_load()
                self.click((By.XPATH, "(//button[@data-bs-target='#deletePropertyModal'])[1]"))
                time.sleep(1)
                self.click(self.deleteSubPropertyBtn)
                self.wait_for_page_load()
                time.sleep(4)
    ...
    def createSTB_Manually_8devices_sequence_(self):
        self.click(self.addBtn)
        for i in range(1, 9):
            global r_roomNum
            global r_serialNum
